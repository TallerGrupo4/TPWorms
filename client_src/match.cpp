...
Match::Match(Snapshot snpsht, MatchSurfaces& surfaces, SDL2pp::Renderer& renderer,
             SDL2pp::Mixer& mixer):
        bkgrnd(std::make_shared<Background>(snpsht.platforms, snpsht.map_dimensions.width,
                                            snpsht.map_dimensions.height,
                                            snpsht.map_dimensions.water_level, surfaces, renderer)),
        ...
        camera(renderer, surfaces, turn_time, my_army_id, snpsht.map_dimensions.width,
               snpsht.map_dimensions.height),
        charge_for_weapon(0),
        timer_for_weapon(0),
    ...
    for (WormSnapshot worm_snpsht: snpsht.worms) {
        std::pair<std::reference_wrapper<SDL2pp::Surface>, SDL_Color> widgets_info =
                (worm_snpsht.team_id > 3) ? team_info.at(4) : team_info.at(worm_snpsht.team_id);
        ...
        std::shared_ptr<Worm> worm = std::make_shared<Worm>(
                worm_snpsht, snpsht.map_dimensions.worm_width, snpsht.map_dimensions.worm_height,
                effects_an, effects_sound, widgets, surfaces, renderer);
        this->worms_map[worm_snpsht.id] = worm;
    }

    for (auto& provision_box_snpsht: snpsht.provision_boxes) {
        std::shared_ptr<ProvisionBox> provision_box = std::make_shared<ProvisionBox>(
                provision_box_snpsht, effects_an, effects_sound, surfaces, renderer);
...
void Match::update_from_snapshot(Snapshot& snpsht, MatchSurfaces& surfaces,
    ...
    if (snpsht.get_end_game() and !snpsht.worms.empty()) {
        ...
        winner_team_id == my_army_id ? effects_sound->play_match_winner_sound() :
                                       effects_sound->play_match_loser_sound();
    }
    ...
    for (auto it = projectiles_map.begin(); it != projectiles_map.end();) {
        if (it->second->get_proj_state() == EXPLODED) {
            Target new_target = {PlayerType, -1, -1, camera.get_offset_x(), camera.get_offset_y()};
    ...
    for (auto& projectile_snpsht: snpsht.projectiles) {
        ...
        if (!projectile) {
            projectile = std::make_shared<Projectile>(projectile_snpsht, effects_an, effects_sound,
                                                      surfaces, renderer);
        } else {
    ...
    for (auto& provision_box_snpsht: snpsht.provision_boxes) {
        ...
        if (!provision_box) {
            provision_box = std::make_shared<ProvisionBox>(provision_box_snpsht, effects_an,
                                                           effects_sound, surfaces, renderer);
        } else {
...
bool Match::get_next_target(Target& new_target) {
    for (auto& worm_pair: worms_map) {
        ...
        if ((worm->get_worm_state() != STILL) && (worm->get_worm_state() != DEAD)) {
            new_target = {
                    WormType,
                    worm_pair.first,
                    -1,
...
void Match::update_camera_for_less_priority_targets(int camera_offset_x, int camera_offset_y,
                                                    bool center_camera) {
    Target new_target;
    if (center_camera) {
        new_target = {WormType, get_turn_worm_id(), -1, get_turn_worm_x() * RESOLUTION_MULTIPLIER,
                      get_turn_worm_y() * RESOLUTION_MULTIPLIER};
        camera.update(new_target);
    } else if (get_next_target(new_target)) {
        camera.update(new_target);
    } else if ((camera_offset_x != 0) && (camera_offset_y != 0)) {
        new_target = {PlayerType, -1, -1,
...
void Match::update_camera(int camera_offset_x, int camera_offset_y, bool center_camera,
                          bool player_activated, bool need_to_be_player_activated,
                          SDL2pp::Window* window, int pos_x_to_warp_mouse,
    ...
    switch (target_type) {
        case WormType: {
            ...
            if ((target_worm->get_worm_state() != STILL) and
                (target_worm->get_worm_state() != DEAD)) {
                new_target = {
                        WormType,
        ...
        case NoneType:
        case PlayerType:
            update_camera_for_less_priority_targets(camera_offset_x, camera_offset_y,
                                                    center_camera);
            break;
        case ProjectileType:
...
bool Match::handle_up_button(std::shared_ptr<Action>& action) {
    if (is_turn_worm_in_my_army() and is_turn_worm_aiming_weapon()) {
        action = std::make_shared<ActionAim>(turn_worm_facing_left() ? LEFT : RIGHT, UP,
                                             worm_turn_id);
        return true;
    }
    return false;
...
bool Match::handle_down_button(std::shared_ptr<Action>& action) {
    if (is_turn_worm_in_my_army() and is_turn_worm_aiming_weapon()) {
        action = std::make_shared<ActionAim>(turn_worm_facing_left() ? LEFT : RIGHT, DOWN,
                                             worm_turn_id);
        return true;
    }
    return false;
...
bool Match::handle_space_button_pressed(std::shared_ptr<Action>& action, bool first_time_pressed) {
    if (is_turn_worm_in_my_army()) {
        if (turn_worm_has_charging_weapon() and is_turn_worm_aiming_weapon() and
            (!already_shot_charged_weapon)) {
            if (first_time_pressed)
                effects_sound->play_powerup_sound();
            charge_for_weapon += 2;
...
bool Match::handle_mouse_right_click(std::shared_ptr<Action>& action, int mouse_x, int mouse_y) {
    if (is_turn_worm_in_my_army()) {
        if (is_turn_worm_still() and turn_worm_has_weapon_to_aim()) {
            ...
            action = std::make_shared<ActionAim>(turn_worm_facing_left() ? LEFT : RIGHT, CENTER,
                                                 worm_turn_id);
            return true;
...
bool Match::handle_mouse_scroll_up(std::shared_ptr<Action>& action) {
    if (is_turn_worm_in_my_army() and
        ((is_turn_worm_still() and !turn_worm_has_guided_weapon()) or
         (is_turn_worm_still() and turn_worm_has_guided_weapon() and !camera.is_marker_active()))) {
...
void Match::handle_mouse_motion(int mouse_x, int mouse_y) {
    if (is_turn_worm_in_my_army() and is_turn_worm_still() and turn_worm_has_guided_weapon() and
        camera.is_marker_active() && (!camera.is_marker_set())) {
        camera.update_marker(mouse_x, mouse_y);
    }
}

bool Match::handle_enter_button(std::shared_ptr<Action>& action) {
    if (is_turn_worm_in_my_army() and (!is_turn_worm_aiming_weapon()) and
        (!turn_worm_has_weapon())) {
...
int Match::get_turn_worm_x() { return worms_map.at(worm_turn_id)->get_worm_x(); }

int Match::get_turn_worm_y() { return worms_map.at(worm_turn_id)->get_worm_y(); }

bool Match::turn_worm_facing_left() { return worms_map.at(worm_turn_id)->worm_facing_left(); }

char Match::get_turn_worm_id() { return worm_turn_id; }

bool Match::is_turn_worm_in_my_army() {
    return worms_map.at(worm_turn_id)->get_army_id() == my_army_id;
}

bool Match::is_turn_worm_still() { return worms_map.at(worm_turn_id)->get_worm_state() == STILL; }

bool Match::turn_worm_has_dynamite() { return worms_map.at(worm_turn_id)->has_dynamite(); }

bool Match::turn_worm_has_weapon() { return worms_map.at(worm_turn_id)->has_weapon(); }
...
bool Match::turn_worm_has_timer_weapon() { return worms_map.at(worm_turn_id)->has_timer_weapon(); }
