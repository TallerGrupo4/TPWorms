...
MatchRenderer::MatchRenderer(Client& client, const Snapshot& map_received):
        ...
        ttf(),
        mixer(AUDIO_HZ_CD_QUALITY, MIX_DEFAULT_FORMAT, MIXER_CHANNELS, MIXER_CHUNKSIZE),
        window("WORMS", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 1280, 720,
        ...
        switch (event.type) {
            case SDL_KEYDOWN: {
                ...
                switch (keyEvent.keysym.sym) {
                    case SDLK_ESCAPE: {
                        match.update_camera(
                                mouse_motion_x, mouse_motion_y, true, true, false, &window,
                                match.get_turn_worm_x() + window.GetWidth() / 2 - mouse_motion_x,
                                match.get_turn_worm_y() + window.GetHeight() / 2 - mouse_motion_y);
                        mouse_motion_x = 0;
                        mouse_motion_y = 0;
                        break;
                    }
            ...
            case SDL_MOUSEBUTTONDOWN: {
                ...
                switch (mouseButtonEvent.button) {
                    ...
                    case SDL_BUTTON_RIGHT:
                        if (match.handle_mouse_right_click(action, mouseButtonEvent.x,
                                                           mouseButtonEvent.y)) {
                            client.send_action(action);
                        }
                        break;
...
void MatchRenderer::execute_and_update(int iter) {
    try {
        ...
        std::cerr << "Uknown execption catched in Main thread, match_renderer " << e.what()
                  << std::endl;
        running = false;
    } catch (...) {
...
void MatchRenderer::start() {
    ...
    Clock clock(std::bind(&MatchRenderer::execute_and_update, this, std::placeholders::_1),
                FRAME_TIME, running);
