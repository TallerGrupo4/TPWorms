...
Worm::Worm(const WormSnapshot& worm_snpsht, int worm_width, int worm_height,
...
int Worm::get_worm_state() { return this->state; }

int Worm::get_worm_x() { return this->x; }

int Worm::get_worm_y() { return this->y; }

TOOLS Worm::get_weapon() { return this->weapon; }

int Worm::get_weapon_ammo() { return this->weapon_ammo; }

bool Worm::has_weapon() { return this->weapon != TOOLS::NO_TOOL; }

bool Worm::has_dynamite() { return this->weapon == TOOLS::DYNAMITE; }
...
bool Worm::has_timer_weapon() {
    ...
    bool is_weapon_airstrike = (this->weapon == TOOLS::AIRSTRIKE);
    bool is_weapon_teleport = (this->weapon == TOOLS::TELEPORTATION);
    return (!(dont_have_weapon or is_weapon_bazooka or is_weapon_mortar or is_weapon_baseball_bat or
              is_weapon_airstrike or is_weapon_teleport));
}
...
bool Worm::worm_facing_left() { return this->facing_left; }

char Worm::get_army_id() { return this->army_id; }
...
void Worm::update_from_snapshot(SDL2pp::Renderer& renderer, WormSnapshot& worm_snpsht) {
    ...
    switch (state) {
        ...
        case BACKFLIPPING:
            if (old_state == STILL)
                effects_sound->play_worm_jump_sound();
            break;
        case DAMAGED:
...
void Worm::render(SDL2pp::Renderer& renderer, int camera_offset_x, int camera_offset_y) {
    ...
    worm_an.render(state, angle, weapon, weapon_ammo, renderer,
                   SDL2pp::Rect(top_left_x, top_left_y, width * RESOLUTION_MULTIPLIER,
                                height * RESOLUTION_MULTIPLIER),
                   facing_left, WORM_WALK_LEFT_OFFSET, WORM_WALK_RIGHT_OFFSET,
                   WORM_WALK_ABOVE_OFFSET, WORM_WALK_BELLOW_OFFSET);
}

void Worm::render_texts_and_widgets(SDL2pp::Renderer& renderer, int camera_offset_x,
                                    int camera_offset_y) {
    int center_x =
            x * RESOLUTION_MULTIPLIER + (int)(renderer.GetLogicalWidth() / 2) - camera_offset_x;
    int top_y = (y - height / 2) * RESOLUTION_MULTIPLIER + (int)(renderer.GetLogicalHeight() / 2) -
                camera_offset_y;

    if (state != DEAD) {
        worm_texts.render(renderer, state, facing_left, center_x,
                          top_y + height * RESOLUTION_MULTIPLIER / 2, top_y,
                          top_y + height * RESOLUTION_MULTIPLIER);
    }
}
