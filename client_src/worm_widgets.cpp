...
WormWidgets::WormWidgets(SDL2pp::Renderer& renderer, char army_id,
        ...
        crosshair_y(0),
        army_text(renderer,
                  SDL2pp::Font(WORMS_FONT_PATH, WORM_FONT_SIZE)
                          .RenderText_Blended(std::string("Player " + std::to_string(army_id + 1)),
                                              player_color)),
        worm_text(renderer,
                  SDL2pp::Font(WORMS_FONT_PATH, WORM_FONT_SIZE)
                          .RenderText_Blended(std::string("Worm " + std::to_string(worm_id + 1)),
                                              player_color)),
        health_text(SDL2pp::Font(WORMS_FONT_PATH, WORM_FONT_SIZE)
                            .RenderText_Blended(std::string(std::to_string(health_points) + "/" +
...
void WormWidgets::update_crosshair(int angle) { this->crosshair_angle = angle; }

void WormWidgets::update_crosshair_from_iter() { this->crosshair_an.update_once(); }

void WormWidgets::render(SDL2pp::Renderer& renderer, int worm_state, bool facing_left,
                         int worm_center_x, int worm_center_y, int worm_top_y, int worm_botom_y) {
...
void WormWidgets::renderTextBackground(SDL2pp::Renderer& renderer, int x, int y,
                                       SDL2pp::Texture& text) {
...
void WormWidgets::handleAimingState(SDL2pp::Renderer& renderer, bool facing_left, int worm_center_x,
                                    int worm_center_y) {
    SDL_RendererFlip flip = facing_left ? SDL_FLIP_NONE : SDL_FLIP_HORIZONTAL;
    int distance_from_worm_center_x = std::round(cos(std::abs(this->crosshair_angle) * M_PI / 180) *
                                                 CROSSHAIR_DISTANCE_FROM_WORM_CENTER);
    int distance_from_worm_center_y = std::round(sin(std::abs(this->crosshair_angle) * M_PI / 180) *
                                                 CROSSHAIR_DISTANCE_FROM_WORM_CENTER);
    ...
    this->crosshair_an.render(
            renderer,
            SDL2pp::Rect(worm_center_x + distance_from_worm_center_x -
                                 this->crosshair_an.get_frame_size() / 2,
                         worm_center_y + distance_from_worm_center_y -
                                 this->crosshair_an.get_frame_size() / 2,
                         this->crosshair_an.get_frame_size(), this->crosshair_an.get_frame_size()),
            flip);
}

int WormWidgets::get_crosshair_x() { return this->crosshair_x / RESOLUTION_MULTIPLIER; }

int WormWidgets::get_crosshair_y() { return this->crosshair_y / RESOLUTION_MULTIPLIER; }
