...
void EffectsAnimations::set_worm_hit_an(SDL2pp::Renderer& renderer, int x, int y) {
    std::unique_ptr<PosAndAnimation> worm_hit_an = std::make_unique<PosAndAnimation>(
            x, y, renderer, worm_hit_surface, MILISECS_FOR_WORM_HIT_SPRITES, true);
...
void EffectsAnimations::set_baseball_bat_hit(SDL2pp::Renderer& renderer, int x, int y) {
    std::unique_ptr<PosAndAnimation> baseball_bat_hit_an = std::make_unique<PosAndAnimation>(
            x, y, renderer, baseball_bat_hit_surface, MILISECS_FOR_BASEBALL_BAT_HIT_SPRITES, true);
...
void EffectsAnimations::set_big_explosion_an(SDL2pp::Renderer& renderer, int x, int y,
                                             int explosion_size) {
    std::unique_ptr<PosAndAnimation> big_circle_explosion_an = std::make_unique<PosAndAnimation>(
            x, y, renderer, big_explosion_circle_surface, MILISECS_FOR_EXPLOSIONS_SPRITES, true,
            explosion_size);
    lingering_animations.push_back(std::move(big_circle_explosion_an));
    std::unique_ptr<PosAndAnimation> big_ring_explosion_an = std::make_unique<PosAndAnimation>(
            x, y, renderer, big_explosion_rings_surface, MILISECS_FOR_EXPLOSIONS_SPRITES, true,
            explosion_size);
...
void EffectsAnimations::set_small_explosion_an(SDL2pp::Renderer& renderer, int x, int y,
                                               int explosion_size) {
    std::unique_ptr<PosAndAnimation> small_circle_explosion_an = std::make_unique<PosAndAnimation>(
            x, y, renderer, small_explosion_circle_surface, MILISECS_FOR_EXPLOSIONS_SPRITES, true,
            explosion_size);
    lingering_animations.push_back(std::move(small_circle_explosion_an));
    std::unique_ptr<PosAndAnimation> small_ring_explosion_an = std::make_unique<PosAndAnimation>(
            x, y, renderer, small_explosion_rings_surface, MILISECS_FOR_EXPLOSIONS_SPRITES, true,
            explosion_size);
...
void EffectsAnimations::set_missile_exhaust_an(SDL2pp::Renderer& renderer, int x, int y) {
    std::unique_ptr<PosAndAnimation> missile_exhaust_an = std::make_unique<PosAndAnimation>(
            x, y, renderer, missile_exhaust_surface, MILISECS_FOR_EXHAUST_SPRITES, true);
