...
public:
    int start(){
        try {
            ...
            Texture sprites(renderer, "sprites/Worms/wwalk.png");
                ...
                while (SDL_PollEvent(&event)) {
                    if (event.type == SDL_QUIT) {
                        ...
                        case SDLK_ESCAPE: case SDLK_q:
                            return 0;
                        case SDLK_RIGHT: {looking_left = false; is_running = true; break;}
                        case SDLK_LEFT: {looking_left = true; is_running = true; break;}
                        }
                    } else if (event.type == SDL_KEYUP) {
                        switch (event.key.keysym.sym) {
                        case SDLK_RIGHT: {looking_left = false; is_running = false; break;}
                        case SDLK_LEFT: {looking_left = true; is_running = false; break;}
                        }
                    }
                }
