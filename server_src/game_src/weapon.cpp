#include "weapon.h"
   ...
   Weapon::Weapon(uint8_t type , int ammo, int max_ammo, int damage , int radius , int timer , int fragments , int fragment_damage , bool is_afected_by_wind , bool can_aim , bool has_potency) :
        Tool(type, ammo, can_aim, max_ammo), damage(damage), radius(radius), timer(timer), fragments(fragments), fragment_damage(fragment_damage), is_afected_by_wind(is_afected_by_wind), has_potency(has_potency) {}
