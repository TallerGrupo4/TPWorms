...
Weapon::Weapon(uint8_t type, int ammo, int max_ammo, int damage, int radius, int fragments,
               ExplosionType explosion_type, ProjectileTypes projectile_type,
               bool is_afected_by_wind, bool can_aim, bool has_potency):
        Tool(type, ammo, max_ammo, can_aim),
        damage(damage),
        radius(radius),
        fragments(fragments),
        explosion_type(explosion_type),
        projectile_type(projectile_type),
        is_afected_by_wind(is_afected_by_wind),
        has_potency(has_potency) {}

b2Body* Weapon::create_projectile_body(b2World* world, float angle, float x, float y,
...
std::shared_ptr<Projectile> Weapon::create_projectile(b2Body* worm, int direction, float angle,
                                                      int power, int timer, float restitution,
                                                      float density) {
    ...
    b2Body* projectileBody =
            gamebuilder.create_projectile_body(angle, pos_x, pos_y, restitution, density);
    std::shared_ptr<Projectile> p = std::make_shared<Projectile>(
            projectileBody, damage, radius, projectile_type, explosion_type, timer, fragments,
            angle, is_afected_by_wind);
