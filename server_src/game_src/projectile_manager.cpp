...
bool ProjectileManager::has_projectiles() { return !projectiles.empty(); }
...
float ProjectileManager::get_wind_force() { return wind_force; }

void ProjectileManager::apply_wind(Projectile& projectile) {
    if (projectile.is_affected_by_wind()) {
        projectile.get_body()->ApplyForceToCenter(
                b2Vec2(wind_force * projectile.get_body()->GetMass(), 0), true);
    }
}
...
void ProjectileManager::reset_id() { projectile_id = 0; }
