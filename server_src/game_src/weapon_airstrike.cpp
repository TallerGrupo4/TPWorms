...
Airstrike::Airstrike():
        Weapon(AIRSTRIKE, AIRSTRIKE_MAX_AMMO / 2, AIRSTRIKE_MAX_AMMO, AIRSTRIKE_DAMAGE,
               AIRSTRIKE_RADIUS, 0, EXPLOSIVE, AirStrikeProj, true, false, false) {}
...
std::shared_ptr<Projectile> Airstrike::shoot_airstrike_projectile(b2Body* worm, float x, float y) {
    ...
    return std::make_shared<Projectile>(body, damage, radius, projectile_type, explosion_type, 0,
                                        fragments, -90 * DEGTORAD, true);
}
...
bool Airstrike::use(b2Body* worm, int direction, float angle, int time, int power, float x, float y,
    ...
    for (int i = 0; i < 6; i++) {
        ...
        projectiles.add_projectile(
                shoot_airstrike_projectile(worm, pos_x + x, roof_height + pos_y));
    }
    return true;
}
