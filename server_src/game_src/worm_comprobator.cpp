...
void WormComprobator::check_during_game(const std::list<std::shared_ptr<Worm>>& worms, int& time,
...
std::list<char> WormComprobator::check_post_game(std::list<std::shared_ptr<Worm>>& worms,
                                                 b2World& world) {
...
bool WormComprobator::have_worm_finished_turn(std::list<std::shared_ptr<Worm>>& worms,
                                              int& cleaning_time) {
    for (auto& w: worms) {
        if (w->get_state() == DEAD) {
            continue;
        ...
        w->store_tool();
        if (w->get_state() != DEAD && w->body->GetLinearVelocity() != b2Vec2_zero ||
            w->body->GetAngularVelocity() != 0) {
            cleaning_time = 1 * FPS;
            return false;
        }
    }
...
void WormComprobator::check_states(Worm& w, int& turn_time, const int& current_turn_player_id) {
    ...
    if (w.get_state() == SHOOTED || w.get_state() == AIMING) {
        return;
    }
    ...
    if (diff < -1.5 && w.get_state() != FALLING && !w.in_contact() &&
        w.body->GetLinearVelocity().Length() > 1) {
        w.set_state(FALLING);
    }
...
bool WormComprobator::check_out_of_map_worm(Worm& w, int& turn_time,
                                            const int& current_turn_player_id, const int& height,
                                            const int& width, const int& water_level) {
