...
Projectile::Projectile(b2Body* body, int damage, int radius, ProjectileTypes type,
                       ExplosionType explosion_type, int timer, int fragments, float angle,
                       bool affected_by_wind):
        Entity(body, PROJECITLE),
        damage(damage),
        radius(radius),
        projectile_type(type),
        explosion_type(explosion_type),
        radius_body_size(0.2),
        timer(timer),
        fragments(fragments),
        angle(angle),
        state(ALIVE),
...
bool Projectile::is_affected_by_wind() { return affected_by_wind; }
...
int Projectile::get_timer() { return timer; }
...
ExplosionType Projectile::get_explosion_type() { return explosion_type; }

void Projectile::set_state(char state) { this->state = state; }

int Projectile::get_state() { return state; }

ProjectileTypes Projectile::get_projectile_type() { return projectile_type; }

int Projectile::get_fragments() { return fragments; }

int Projectile::get_fragment_damage() { return FRAGMENT_DAMAGE; }
...
void Projectile::set_angle(float angle) { body->SetTransform(body->GetPosition(), angle); }

void Projectile::set_id(char id) { this->id = id; }

char Projectile::get_id() { return id; }

ProjectileSnapshot Projectile::get_snapshot() {
    float pos_x = body->GetPosition().x;
    float pos_y = body->GetPosition().y;
    return ProjectileSnapshot(projectile_type, pos_x, pos_y, get_angle(), get_direction(),
                              radius_body_size, state, id, radius);
}
