...
public:
    ...
    Snapshot(const std::vector<WormSnapshot>& worms,
             const std::vector<ProjectileSnapshot>& projectiles,
             const std::vector<PlatformSnapshot>& platforms,
             const std::vector<ProvisionBoxSnapshot>& provision_boxes):
            worms(worms),
            projectiles(projectiles),
            platforms(platforms),
            provision_boxes(provision_boxes),
            map_dimensions() {}

    Snapshot() {}
    ~Snapshot() {}

    void set_dimensions(float height = 0, float width = 0, float worm_width = WORM_WIDTH,
                        float worm_height = WORM_HEIGHT, int amount_of_worms = 0,
                        int water_level = 0) {
    ...
    void set_armies_health(const std::map<char, int>& armies_health) {
        this->armies_health = armies_health;
    }

    void set_end_game() { end_game = true; }

    void set_wind_force(int wind_force) { this->wind_force = wind_force; }

    int get_wind_force() { return wind_force; }
