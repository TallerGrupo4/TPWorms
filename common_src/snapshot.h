...
class ProjectileSnapshot {
    ...
    ProjectileSnapshot(int type, float pos_x, float pos_y, float angle, int direction, float radius , int state, char id, int explosion_type):
            type(type), pos_x(pos_x), pos_y(pos_y), angle(angle), direction(direction), state(state), id(id), explosion_type(explosion_type), radius(radius) {};
